...
@interface RSCreateQRViewController ()
...
@implementation RSCreateQRViewController
...
- (IBAction)createBtnClick:(id)sender {
    ...
    if (self.textView.text.length > 0) {
        self.imgQRView.image = [self createQRImageWithString:self.textView.text size:qrImageSize];
...
- (IBAction)saveBtnClick:(id)sender {
    UIImageWriteToSavedPhotosAlbum(self.imgQRView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), (__bridge void *)self);
}
...
- (IBAction)changeColorClick:(id)sender {
    ...
    self.imgQRView.image = [self changeColorForQRImage:image backColor:kRandomColor frontColor:kRandomColor];
}
